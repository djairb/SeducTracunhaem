  ...
  return (
    <div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-conectiva-navy">Alunos Matriculados</h1>
          <p className="text-gray-500">Gerencie o corpo discente ({alunos.length} total).</p>
        </div>
        
        <Link 
          to="/alunos/novo" 
          className="bg-conectiva-navy text-white px-4 py-2.5 rounded-lg flex items-center gap-2 font-medium hover:bg-slate-800 transition shadow-lg shadow-conectiva-navy/20"
        >
          <UserPlus size={20} />
          Nova Matrícula
        </Link>
            ...
            <div key={aluno.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-all duration-200 group relative flex flex-col justify-between">
              ...
              <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                <Link 
                    ...
                    className="p-2 bg-white text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-full transition border border-gray-100 shadow-sm" 
                    title="Editar"
                >
                    <Edit2 size={16} />
                </Link>
              ...
              <div>
                <div className="flex items-start gap-4 mb-4">
                    <div className="w-12 h-12 rounded-full bg-conectiva-navy text-conectiva-lime flex items-center justify-center font-bold text-lg shrink-0">
                ...
                <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-sm text-gray-600 mt-2 bg-gray-50 p-3 rounded-lg border border-gray-100">
                    <div className="flex items-center gap-2"><User size={14} className="text-conectiva-navy" /><span className="capitalize">{aluno.sexo}</span></div>
                    <div className="flex items-center gap-2"><Calendar size={14} className="text-conectiva-navy" /><span>{calcularIdade(aluno.data_nascimento)} anos</span></div>
                    <div className="flex items-center gap-2 col-span-2"><Fingerprint size={14} className="text-conectiva-navy" /><span className="font-mono text-xs">{formatarCPF(aluno.cpf)}</span></div>
                    <div className="flex items-center gap-2 col-span-2"><Phone size={14} className="text-conectiva-navy" /><span>{aluno.telefone_celular}</span></div>
                </div>
              </div>

              <div className="mt-4 pt-3 border-t border-gray-100 flex justify-between items-center">
                 <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase ${
                    aluno.status_financeiro === 'Em dia' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                 }`}>

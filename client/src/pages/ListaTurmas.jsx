  ...
  return (
    <div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-seduc-primary">Gestão de Turmas</h1>
          <p className="text-gray-500">Administre os períodos letivos e enturmação.</p>
        </div>
        <button
          onClick={() => setModalAberto(true)}
          className="bg-seduc-primary text-white px-4 py-2.5 rounded-lg flex items-center gap-2 font-bold hover:bg-white hover:text-seduc-primary border border-transparent hover:border-seduc-primary shadow-lg transition"
        >
          <Plus size={20} />
          Abrir Nova Turma
        </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {turmas.map((turma) => (
            <div key={turma.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden group hover:shadow-md transition-all">
              ...
              <div className={`h-2 w-full ${turma.ativa ? 'bg-seduc-primary' : 'bg-gray-300'}`} />
              ...
              <div className="p-6">
                ...
                <div className="space-y-2 text-sm text-gray-600 mb-6">
                  <div className="flex items-center gap-2">
                    <Calendar size={16} className="text-seduc-primary" />
                    <span>{turma.periodo}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Clock size={16} className="text-seduc-primary" />
                    <span>{turma.turno}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Users size={16} className="text-seduc-primary" />
                ...
                <div className="flex gap-2">
                  <Link
                    to={`/turmas/${turma.id}`}
                    className="flex-1 bg-seduc-primary text-white py-2 rounded-lg flex items-center justify-center gap-2 text-sm font-medium hover:bg-white hover:text-seduc-primary border border-transparent hover:border-seduc-primary transition"
                  >
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6">
            ...
            <form onSubmit={handleSubmit(criarTurma)} className="space-y-4">
              ...
              <button
                ...
                className="w-full bg-seduc-secondary text-seduc-primary font-bold py-3 rounded-lg hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed mt-4 shadow-lg border border-seduc-primary"
              >

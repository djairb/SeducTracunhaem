  ...
  return (
    <div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-seduc-primary">Corpo Docente</h1>
          <p className="text-gray-500">Gestão dos Professores ({professores.length} ativos).</p>
        </div>
        <Link to="/professores/novo" className="bg-seduc-primary text-white px-4 py-2.5 rounded-lg flex items-center gap-2 font-medium hover:bg-white hover:text-seduc-primary border border-transparent hover:border-seduc-primary shadow-lg shadow-seduc-primary/20 transition">
          ...
          {exibidos.map((prof) => (
            <div key={prof.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-all group relative flex flex-col justify-between">
              ...
              <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                <Link to={`/professores/editar/${prof.id}`} className="p-2 bg-white text-gray-400 hover:text-seduc-primary hover:bg-green-50 rounded-full border border-gray-100 shadow-sm"><Edit2 size={16} /></Link>
